...
const child_id = new mongoose.mongo.ObjectId();
const parent_id = new mongoose.mongo.ObjectId();
...
describe("Auth Route Endpoints", () => {
  ...
  after((done) => {
    ...
    Auth.findByIdAndDelete(parent_id);
    Auth.findByIdAndDelete(child_id);
    done();
  });
  ...
  describe("POST api/v1/auth/sign-up-for-a-child", () => {
    before(async () => {
      await Auth.create({
        ...customParent,
      });
    ...
    after(async () => {
      await Auth.findByIdAndDelete(parent_id);
    });
    it("should not create account if the user supplies incomplete information", (done) => {
      chai
        .request(app)
